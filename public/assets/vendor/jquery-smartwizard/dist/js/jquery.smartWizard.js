  ...
  var SmartWizard = /*#__PURE__*/function () {
    ...
    _createClass(SmartWizard, [{
      ...
      value: function _initLoad() {
        ...
        // Add a delegated click listener for nav links to update button state
        // (ensures update when user clicks a tab directly)
        var self = this;
        // Avoid duplicate attachment
        if (!this.main.data('sw-nav-click-init')) {
          this.main.data('sw-nav-click-init', true);
          this.main.on("click", ".nav-link", function (e) {
            // let original handler run then update state shortly after
            setTimeout(function () {
              // refresh steps collection (in case dynamic)
              self.steps = self.nav.find('.nav-link');
      ...
      value: function _createToolbar(position) {
        ...
        // Buat tombol Next
        var btnNext = this.options.toolbarSettings.showNextButton !== false
          ? $('<button></button>')
        ...
        var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false
          ? $('<button></button>')
        ...
        // Simpan tombol di instance agar tetap bisa diakses
        this.btnNext = btnNext;
        this.btnPrev = btnPrevious;

        // Coba cari modal footer terdekat dari this.main
        var modal = this.main.closest('.modal');
        var modalFooter = null;

        if (modal && modal.length) {
          var mf = modal.find('.modal-footer');
          if (mf && mf.length) modalFooter = mf.get(0);
        }

        if (modalFooter) {
          // Pindahkan tombol ke footer modal
          modalFooter.appendChild(btnPrevious[0]);
          modalFooter.appendChild(btnNext[0]);

          // Tambahkan event klik ulang karena elemen berpindah DOM
          var self = this;
          ...
          // Fallback: tetap tempel ke dalam toolbar default
          toolbar.append(btnPrevious, btnNext);
        }

        // Tambahkan tombol ekstra jika ada
    ...
    },
    {
      ...
      value: function _updateButtonState() {
        // refresh steps in case they are dynamic
        this.steps = this.nav.find('.nav-link');

        var totalSteps = this.steps.length;
        var currentIndex = this.current_index;
        ...
        if (currentIndex === totalSteps - 1) {
          // Step terakhir → ubah teks menjadi "Simpan", tapi warna tetap btn-primary
          this.btnNext
            .text("Simpan")
            .removeClass("btn-success")
            .addClass("btn-primary")
            .attr("type", "button") // keep as button to avoid auto-submit
            .off("click")
            .on("click", function () {
              // Cari form di dalam modal (tanpa bergantung pada ID)
              ...
              if (form.length) {
                ...
                console.warn("Tidak ditemukan elemen <form> di modal ini.");
              }
            });
        } else {
          // Step selain terakhir → tombol Next normal
          this.btnNext
            .text("Selanjutnya")
            .removeClass("btn-success")
            .addClass("btn-primary")
    ...
    },
    {
      ...
      value: function _transitStep(idx) {
        ...
        // ensure our custom state updated as well
        this._updateButtonState();
      }
    }, {
